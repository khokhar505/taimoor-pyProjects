...
player=Entity(name='O',color=color.violet)
cursor=Tooltip(color=player.color,origin=(0,0),scale=4,enabled=True)
cursor.background.color=color.clear

bg=Entity(parent=scene,model='quad',texture='sky_sunset',scale=(16,8),z=10,color=color.light_gray)
mouse.visible=True
...
def checkforvictory():
    ...
    if won:
        ...
        mouse.visible=True
        Panel(z=1, scale=10, model='quad')
        t = Text(f'Try again!\n{name}\nBest of luck!', scale=3, origin=(0,0), background=True)
        t.create_background(padding=(.5,.25), radius=Text.size/2)
        t.background.color = player.color.tint(-.2)
